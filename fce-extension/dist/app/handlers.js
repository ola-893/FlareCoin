...
import { decodeRebalanceRequest, encodeRebalancePayload, encodeStrategyAPYs, PARENT_VAULT_ABI, STRATEGY_ADAPTER_ABI, } from "./abi.js";
import { COSTON2_RPC_URL, FTSO_ADAPTER, MIN_REBALANCE_AMOUNT, MIN_TWAP_WINDOW, SLIPPAGE_TOLERANCE_BPS, SPARKDEX_ADAPTER, ENOSYS_CDP_ADAPTER, } from "./config.js";
const state = {
    lastRebalanceTime: 0,
    totalRebalances: 0,
    lastOptimalStrategy: "",
 ...
 */
export async function handleCalculateOptimal(msg) {
    ...
    let request;
    try {
        request = decodeRebalanceRequest(hex);
    }
    catch (e) {
 ...
 */
export async function handleGetAPYs(msg) {
    ...
    // 2. Calculate APYs
    const apys = await calculateStrategyAPYs(strategies);
    if (apys.length === 0) {
 ...
 */
async function calculateStrategyAPYs(strategies) {
 ...
 */
function selectOptimalStrategy(apys) {
    if (apys.length === 0) {
 ...
 */
async function getCurrentNonce(vaultAddress) {
    try {
        const client = createPublicClient({
            transport: http(COSTON2_RPC_URL),
        });
